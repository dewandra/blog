            ...
            <div class="row">
                                ...
                                <div class="col-md-6" data-aos="fade">
                                    <article class="blog-post">
                                        <div class="post-slider slider-sm rounded">
                                            <img loading="lazy" decoding="async" src="{{ asset('storage/'.$a->image) }}" alt="Post Thumbnail">
                                            
                                        </div>
                                        <div class="pt-4">
                                            <p class="mb-3">{{ \Carbon\Carbon::parse($a->created_at)->format('d M, Y') }}</p>
                                            <h2 class="h4"><a class="text-black" wire:navigate href="{{ route('blog',$a->id) }}">{{ $a->title }}</a></h2>
                                            <p>Heading example Here is example of hedings. You can use this heading by following …</p> <a wire:navigate href="{{ route('blog',$a->id) }}" class="text-primary fw-bold" aria-label="Read the full article by clicking here">Read More</a>
                                        </div>
                                    </article>
                                </div>
                                @endforeach
                ...
                <div class="col-lg-3">
                    ...
                    <div class="widget widget-categories">
                        ...
                        <ul class="list-unstyled widget-list">			
                            @if ($categories->isNotEmpty())
                                ...
                                <li><a wire:navigate href="{{ route('blogs').'?categorySlug='.$c->slug }}">{{ $c->name }}</a>
                                </li>			
                                @endforeach
                            @endif
                        </ul>
                    ...
                    <div class="widget">
                        ...
                        @if ($lastestArticles->isNotEmpty())
                            @foreach ($lastestArticles as $la)
                                <!-- post-item -->
                                <ul class="list-unstyled widget-list">
                                    <li class="d-flex widget-post align-items-center">
                                        <a class="text-black" href="/blog/elements/">
                                            <div class="widget-post-image flex-shrink-0 me-3">
                                                <img loading="lazy" decoding="async" src="{{ asset('storage/'.$la->image) }}" alt="Post Thumbnail">
                                            </div>
                                        </a>
                                        <div class="flex-grow-1">
                                            <h5 class="h6 mb-0"><a class="text-black" href="blog-details.html">{{ $la->title }}</a></h5>
